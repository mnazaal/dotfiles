...
- pytest-xdist runs one process per worker, each independently JIT-compiling; a full parallel suite run can multiply memory/CPU far past a single-process run. Scope to changed test files by default and run the full suite deliberately, not reflexively.
- Before launching any full or slow-marked suite, check available memory (`free -m`) first. A project's hardcoded low `-n` in `addopts` is typically validated only for an idle/solo machine — running the same command while the user is actively using the machine can still freeze it. If memory is tight or the user appears active, wait or ask instead of launching anyway.
- Worker crashes under `-n auto`/high worker counts showing `OpenBLAS blas_thread_init: pthread_create failed ... RLIMIT_NPROC`, `Fatal Python error: Aborted`, or `node down: Not properly terminated` (no `FAILED` line) are resource exhaustion from too many concurrent JIT-compiling processes, not a code bug — rerun the same command with a lower `-n` (e.g. `-n 2`) instead of debugging the code.
