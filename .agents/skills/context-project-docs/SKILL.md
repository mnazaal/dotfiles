...
- Boundary: this skill governs standing project Markdown. The personal Org note store is read-only for agents (AGENTS.md). `learn-topic` may create its own learning-workspace `MISSION.md`/`CONTEXT.md`/`LOG.md` files; those are scoped to the learning workspace, not general project standing docs.
