...
- **Submitted ≠ running — verify before recording a launch as in-progress.** The driver/launcher returning (or a wrapper's `launched (pid …)` line) means submission reached the scheduler, NOT that the job cleared config composition. A job that dies in seconds on a bad override leaves an early non-zero `.exit`/`_result.pkl` and zero output — indistinguishable from "still starting" if you only see that the launcher returned. Before trusting a launch — especially before writing "running" into a `session-handoff` — confirm `squeue`/`sacct` shows `RUNNING`/`COMPLETING` (not `FAILED`/absent) AND the first expected output artifact exists.
...
- Operations needing SSH to a remote — writes (push/clone of a private repo, `uv sync` resolving a git dep, first host-key accept) AND read-only status/pulls (`ssh <host> squeue`/`sacct`, `rsync` of results) — fail in a sandboxed agent session. Hand the exact command to the user (`!` prefix); don't retry it yourself.
