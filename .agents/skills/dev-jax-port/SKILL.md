...
- A reference decision computed by an inner solver you cannot bit-reproduce (an adaptive/sequential design that re-selects each step from a running fit the reference solves with a different optimizer — SciPy Brent vs a JAX Newton) is not replayable: a ~1e-5 fit difference moves the next query and the trajectories diverge by construction. Parity the estimator on replayed inputs (same inputs → same output, to tolerance); verify the selection *policy* statistically (medians/distributions agree over seeds); leave the reference as the oracle for numbers that design produced.
