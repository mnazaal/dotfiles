...
- Never create a merge commit; branches land by fast-forward. If `git merge --ff-only` refuses, the branch has diverged: `git checkout <branch> && git rebase <base>`, then fast-forward. `git merge --no-ff`, a `git pull` on a diverged branch, and a `--merge`-style pull-request merge each write one instead — use fast-forward, rebase or squash. A hook that refuses the merge commit is this rule working; rebase rather than reaching for `--no-verify` or a bypass variable, and let the user decide if a real merge commit is wanted.
...
- A repository may confine agent commits to a branch prefix, and the hook says so when it rejects one. The repair is to branch and commit yourself — `git branch --show-current`, then switch to a fresh prefixed branch — never to unset the guard, add `--no-verify`, or hand the user a commit command for the base branch: the guard exists to route agent work onto agent branches, not to make commits the user's job. After a branch is merged and deleted you are back on the base, so check the current branch before staging rather than after the hook rejects.
...
2. Summarize scope and risk. Before recommending a merge, check it can fast-forward: `git merge-base --is-ancestor <target> <branch>` (exit 0 = a `--ff-only` merge will work). If it cannot, rebase the branch onto the target rather than proposing a merge commit, and dry-run the conflicts first: `git merge-tree $(git merge-base <target> <branch>) <target> <branch>` — inspect for conflict markers without touching the working tree.
