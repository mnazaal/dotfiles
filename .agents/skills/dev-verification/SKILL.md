...
- A multi-case test stops at its first failing assertion, so every case after it is UNEXECUTED — not passing, and not uncovered either. A red run undercounts defects, and "no test covers X" read off that run is wrong whenever the test covering X sits below the failure. Fix the first, re-run, then count.
